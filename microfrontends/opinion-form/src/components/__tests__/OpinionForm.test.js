...
describe('Opinion Form', () => {
  it('shows form', () => {
    ...
    expect(getByLabelText('entities.opinion.created').value).toBe(
      new Date(opinionMock.created).toLocaleString(i18n.language)
    );
    expect(getByLabelText('entities.opinion.text').value).toBe(opinionMock.text);
    expect(getByLabelText('entities.opinion.sentences').value).toBe(
      opinionMock.sentences.toString()
    );
    expect(getByLabelText('entities.opinion.score').value).toBe(opinionMock.score.toString());
    expect(getByLabelText('entities.opinion.result').value).toBe(opinionMock.result);
  });
